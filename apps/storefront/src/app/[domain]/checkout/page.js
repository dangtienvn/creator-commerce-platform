      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div>
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            >
              {loading ? 'Processing...' : `Pay ${getCartTotal().toLocaleString()} â‚«`}
            </button>
          </form>
        </div>
        ...
        <div>
          <div className="bg-gray-50 rounded-2xl border border-gray-100 p-8 sticky top-24">
            ...
            <ul className="space-y-4 mb-6 divide-y divide-gray-200">
              {cartItems.map(item => (
                <li key={item.id} className="pt-4 first:pt-0 flex justify-between">
                  ...
                  <p className="font-medium text-gray-900">{(item.price * item.quantity).toLocaleString()} â‚«</p>
                </li>
              ))}
            </ul>
            ...
            <div className="border-t border-gray-200 pt-4 flex justify-between items-end">
              ...
              <span className="text-3xl font-extrabold text-blue-600">{getCartTotal().toLocaleString()} â‚«</span>
            </div>
          </div>
        </div>
      </div>
