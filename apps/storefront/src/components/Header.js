...
import { ShoppingCart, Search, User, Compass } from 'lucide-react';
import useCartStore from '../store/useCartStore';
import useAuthStore from '../store/useAuthStore';

export default function Header({ isMainPlatform = false, creatorName = "Kernel" }) {
      ...
      <div className="container mx-auto px-6 h-14 flex items-center justify-between">
        ...
        >
          <div className="relative w-8 h-8 rounded-full overflow-hidden border border-gray-200">
            <Image src="/img/avatar.png" alt="Logo" fill sizes="32px" className="object-cover" />
          </div>
        ...
        {isMainPlatform ? (
          <nav className="hidden md:flex items-center gap-8 text-sm font-semibold text-gray-600">
            <Link href="#discover" className="hover:text-gray-900 transition-colors">Discover</Link>
          ...
          <div className="hidden md:flex flex-1 max-w-md mx-8 relative">
            <input 
              type="text" 
              placeholder={`Search ${creatorName}...`}
        ...
        <div className="flex items-center gap-6">
          {isMainPlatform ? (
            <>
              <a href="http://localhost:3001/login" className="text-sm font-semibold text-gray-600 hover:text-gray-900 transition-colors hidden sm:block">
                Log in
              </a>
              <Link href="/register" className="text-sm font-bold text-white bg-black px-5 py-2 rounded-md hover:bg-gray-800 transition-colors shadow-sm">
                Start selling
              </Link>
            </>
          ) : (
            <>
              {!isMainPlatform && (
                <Link href="/blog" className="text-sm font-semibold text-gray-600 hover:text-gray-900 transition-colors hidden sm:block">
                  Blog
                </Link>
              )}
              {mounted && user ? (
